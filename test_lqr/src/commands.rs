    ...
    match byte {
        ...
        b'F' => {
            if BALANCE_ENABLE.load(Ordering::Relaxed) {
                ...
                // Direct-drive mode: spin motors forward (like test_full).
        ...
        b'B' => {
            if BALANCE_ENABLE.load(Ordering::Relaxed) {
                ...
                // Direct-drive mode: spin motors backward (like test_full).
                MOTOR_CMD.store(2, Ordering::Relaxed);
            }
        }
        b'S' => {
            if BALANCE_ENABLE.load(Ordering::Relaxed) {
                // Balanced mode: zero the velocity reference, keep balancing.
                V_REF_MM_S.store(0, Ordering::Relaxed);
            } else {
